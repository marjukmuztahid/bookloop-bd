...
const MyListings = () => {
  ...
  const removeListing = async (id: string) => {
    const { error } = await supabase.from('listings').update({ status: 'deleted' }).eq('id', id).eq('seller_id', user!.id);
    if (error) { 
      console.error('Delete listing error:', error);
      showToast('Failed to delete listing', 'error'); 
      return; 
    }
    showToast('Listing removed from your dashboard', 'success');
        ...
        {confirmDelete && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              <p className="mb-5 text-sm text-[#8A8A8A]">This will remove the listing from your dashboard and the marketplace. You can contact support if you need it restored.</p>
